...
The **output** of the two-factor hazard rate is the *istant probability of default* itself, the *credit spread* and the *corporate bond price term structures*. Other interesting outputs are: 
- the loss level; 
- the arrival rate of the loss; 
- the rate of recovery; 
- the duration gap; 
- the volatility of cash assets. 

Here it is shown the hazard rate or the probability of default, which as expected increases at the worst ratings.
...
## Interesting results
As the model was implemented on empirical data, interesting results emerged. 
- First, the model was able to capture, during the Covid period, an increase in the likelihood of default for hospitality, transport, and retail companies. 
...
- Different optimization options (here it is used a non-linear optimization function `fmincon` with the Levenberg-Marquardt algorithm).
